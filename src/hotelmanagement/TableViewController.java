...
public class TableViewController implements Initializable {
  ...
  public void saveFile(ObservableList<User> userObservableList, File file) {
    try {
      final BufferedWriter outWriter = new BufferedWriter(new FileWriter(file + ".txt"));
    ...
    try {
      ...
      while ((line = br.readLine()) != null) {
        ...
        if (Integer.parseInt(array[5]) == 0) {
          d.addAdmin(new User(array[0], array[1], array[2], array[3], array[4],
                  Integer.parseInt(array[5]), array[6], Integer.parseInt(array[7])));
        } else {
  ...
  public ObservableList<User> getUsers() {
    ...
    users.add(new User("ADMIN", "9999999999", "admin@login.com", "ADMIN",
            d.getUserSecondaryDb(0).getPassword(), 0, "04/23/29"));
